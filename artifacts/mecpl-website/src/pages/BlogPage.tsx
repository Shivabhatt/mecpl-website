...
// Using an inline header here so this page can use an h2 title
    ...
    <div data-animate-page className="bg-mecpl-dark pt-20">
      <section className="max-w-7xl mx-auto px-6 py-16 space-y-8">
        <div className="space-y-3 text-center max-w-3xl mx-auto px-6 pt-6 pb-0">
          ...
          <h1 className="text-5xl font-black tracking-tight uppercase leading-tight text-white">Latest MECPL Posts</h1>
          <p className="text-gray-400 text-sm leading-relaxed">Select a story below to open the full long-form article.</p>
        </div>

        {/* Featured first post - side-by-side layout on md+ */}
        {blogPosts.length > 0 && (
          <article className="group relative overflow-hidden rounded-[24px] border border-white/8 bg-mecpl-card shadow-2xl shadow-black/20 transition-colors hover:border-[#C41E3A]/30">
            <div className="grid md:grid-cols-2">
              <div className="h-72 md:h-auto w-full overflow-hidden">
                <img src={blogPosts[0].heroImage} alt={blogPosts[0].title} className="w-full h-full object-cover opacity-95 group-hover:scale-105 transition-transform duration-700" />
              </div>
              <div className="p-8 md:p-12 flex flex-col justify-center">
                <div className="flex items-center gap-4 mb-3 text-[9px] uppercase tracking-[0.28em] text-white/50">
                  <span>{blogPosts[0].publishedDate}</span>
                  <span>{blogPosts[0].category}</span>
                </div>
                <h3 className="text-3xl font-black uppercase tracking-tight text-white mb-3">{blogPosts[0].title}</h3>
                <p className="text-gray-300 text-base leading-relaxed max-w-3xl">{blogPosts[0].deck}</p>
        ...
        {/* Remaining posts as list-style cards inspired by Oberoi */}
        <div className="mt-10 space-y-6">
          {blogPosts.slice(1).map((post) => (
            <div key={post.slug} className="flex gap-6 items-start bg-mecpl-card border border-white/8 rounded-lg p-4 md:p-6 hover:shadow-xl transition-all">
              <img src={post.heroImage} alt={post.title} className="w-36 h-24 object-cover rounded-sm flex-shrink-0" />
              <div className="flex-1">
                <div className="text-[10px] uppercase text-white/60 tracking-widest mb-1">{post.publishedDate} | {post.category}</div>
                <h3 className="text-lg font-black text-white tracking-tight">{post.title}</h3>
                <p className="text-gray-300 text-sm mt-2">{post.deck}</p>
                <div className="mt-3">
      ...
      <section className="max-w-7xl mx-auto px-6 pb-20">
        <div className="rounded-[28px] border border-white/10 bg-white/5 backdrop-blur-2xl p-8 md:p-10 flex flex-col md:flex-row items-start md:items-center justify-between gap-6">
          <div className="space-y-2 max-w-2xl">
            ...
            <h3 className="text-3xl font-black uppercase tracking-tight text-white">More project notes and company updates are on the way</h3>
