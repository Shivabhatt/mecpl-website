...
export default function Navbar() {
  ...
  const shellClass = isLight
    ? "bg-[#f6f2eb] backdrop-blur-0 border-black/10 shadow-2xl shadow-black/10 md:bg-[#f6f2eb]/95 md:backdrop-blur-2xl"
  ...
  const navTextClass = isLight ? "text-[#1a1a1a]/65 hover:text-[#1a1a1a]" : "text-white/75 hover:text-white";
  const navActiveClass = isLight ? "text-[#1a1a1a]" : "text-white";
  const mutedTextClass = isLight ? "text-[#1a1a1a]/55 hover:text-[#1a1a1a]" : "text-white/55 hover:text-white";
  const mobilePanelClass = isLight
    ? "bg-[#f6f2eb] backdrop-blur-0 border-black/10 shadow-2xl shadow-black/10"
  ...
  return (
    <header
      ...
      style={isMobile ? { backgroundColor: isLight ? "#f6f2eb" : "#111827", backdropFilter: "none", WebkitBackdropFilter: "none" } : undefined}
      data-testid="navbar"
    ...
    >
      <div className="px-4 sm:px-5 md:px-7 h-15 md:h-18 flex items-center justify-between gap-3 md:gap-4">
        <Link href="/" data-testid="link-logo">
          <div className="flex items-center gap-2.5 cursor-pointer group select-none">
            ...
            <div className="leading-none">
              <div className={`font-black text-base md:text-xl tracking-tighter ${isLight ? "text-[#1a1a1a]" : "text-white"}`}>MECPL</div>
              <div className={`text-[7px] md:text-[9px] font-bold tracking-[0.28em] uppercase leading-none mt-1 ${isLight ? "text-[#1a1a1a]/50" : "text-white/50"}`}>
        ...
        <div className="hidden md:flex items-center gap-3">
          ...
          <button
            onClick={toggleTheme}
            className={`w-10 h-10 border bg-transparent flex items-center justify-center transition-all rounded-full ${isLight ? "border-black/10 text-[#1a1a1a]/65 hover:text-[#1a1a1a] hover:border-black/20" : "border-white/15 text-white/70 hover:text-white hover:border-white/30"}`}
          ...
          <button
            onClick={openModal}
            className="bg-[#C41E3A] hover:bg-[#d72b46] text-white text-[11px] font-black tracking-[0.25em] uppercase px-5 py-3 rounded-full transition-colors cursor-pointer border border-[#C41E3A] whitespace-nowrap"
        ...
        <div className="md:hidden flex items-center gap-2">
          <button
            onClick={toggleTheme}
            className={`w-9 h-9 border bg-transparent flex items-center justify-center transition-all rounded-full ${isLight ? "border-black/10 text-[#1a1a1a]/65" : "border-white/15 text-white/70"}`}
        ...
        >
          ...
          <div className="pt-3 border-t border-white/10 space-y-3">
            ...
            <button
              ...
              className="w-full text-center bg-[#C41E3A] border border-[#C41E3A] text-white py-3 text-[10px] font-black tracking-[0.22em] uppercase cursor-pointer mt-2 rounded-full whitespace-nowrap"
